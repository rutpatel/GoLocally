...
//I'm using apiai API to enable small conversations
var appapiai = apiai('Your APIAI Access Token');

var app = express();

var token = "Facebook App Access Token"
var googleStaticMapKey = "Google Static Maps API Key";
var googlePlacesApiKey = "Google Places API Key";
var googleMapsApiKey = 'Google Maps API Key'
var locations;
var jsonData = '';
...
app.get('/webhook/', function (req, res) {
    if (req.query['hub.verify_token'] === "itsmyapp1") {
...
function processTextMessage(sender, text){
    ...
    if((placesArray.indexOf(text) > -1) && userLat != undefined){ 
        ...
        var user = 'https://graph.facebook.com/v2.6/'+sender+'?fields=first_name&access_token='+token;
        ...
        var req = appapiai.textRequest(text, {
            sessionId: 'api.ai Session ID'
        });
        ...
        req.on('response', function(response) {
            ...
            if(response && response.result && response.result.fulfillment && response.result.fulfillment.speech){
                ...
                var apiaiAction = response.result.action;
                if(apiaiAction == 'input.unknown'){
                    sendTextMessage(sender, 'Type in "Help" to learn how to use me! :-) ');
                }
            }
        });
